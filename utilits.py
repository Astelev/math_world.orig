...
class Anim:
    def __init__(self, filename, frames):
        ...
        self.nuwframe = 0
    def framedrow(self, screen, x, y):
        screen.blit(self.images[self.nuwframe], (x, y))
        if self.nuwframe < len(self.images) - 1:
            self.nuwframe += 1
        else:
            self.nuwframe = 0
...
class Word:
    def __init__(self, camerax, cameray, screen, objects=[], florcul=(100, 100, 100), backcol=(0, 0, 0)):
        self.florcul = florcul
    ...
    def display(self):
        ...
        pygame.draw.rect(self.scr, self.florcul, (0, 10-self.camy, 1000, 1000))
    ...
    def click(self, x, y, muveobj=False):
        for i in self.col:
...
class Person:
    ...
    def display(self, x, y, screen):
        if self.do[0] == "move":
            self.run.framedrow(screen, 400, 310)
        else:
